...
func TestInit_FreshDir_CreatesLayout(t *testing.T) {
	...
	// Manifest
	manifestPath := dbPath + "/" + waldb.ManifestFileName
...
func TestInit_AlreadyInitialized_DoesNotCorrupt(t *testing.T) {
	...
	_ = waldb_db.Init(dbPath, logger.NoOpLogger{})
...
func TestInit_ConflictingManifestDir_EarlyRejection(t *testing.T) {
	...
	manifestConflict := dbPath + "/" + waldb.ManifestFileName
	...
	tst.AssertTrue(t, errors.Is(err, waldb_db.ErrAlreadyExists), "expected ErrAlreadyExists")
	...
	tst.AssertTrue(t, os.IsNotExist(statErr),
		"expected lock file to be absent after early Init failure")

	// Open must also fail (not mislead the caller into thinking it is a valid DB).
	_, err = waldb_db.Open(dbPath, logger.NoOpLogger{})
	tst.AssertNotNil(t, err, "expected Open to fail on partially conflicting directory")
}

// AC3: A manually created partial layout (WAL dir + LOCK, no manifest) is
// correctly rejected by both Open and a subsequent Init call, demonstrating
// that partial state does not lead to misleading behaviour.
func TestInit_PartialLayout_RejectedByOpenAndInit(t *testing.T) {
	dbPath := t.TempDir()

	// Simulate what Init would have written before a manifest failure:
	// create the WAL directory and the lock file, but leave out the manifest.
	walDir := dbPath + "/" + waldb.WALDirName
	...
	// Open must fail — the manifest is absent.
	_, err = waldb_db.Open(dbPath, logger.NoOpLogger{})
	tst.AssertNotNil(t, err, "expected Open to fail when manifest is absent")
	tst.AssertTrue(t, errors.Is(err, waldb_db.ErrManifestMissing),
		"expected ErrManifestMissing for partial layout")

	// Init must also fail — the directory is non-empty.
	err = waldb_db.Init(dbPath, logger.NoOpLogger{})
